...
async function generateEmbedding(text: string): Promise<number[]> {
    ...
    // The SDK automatically parses the response into an array of numbers
...
export async function POST(
    ...
    // If absolutely no chunks were processed, stop early to avoid further errors
    if (successfulChunks === 0) {
        return NextResponse.json({ error: "Failed to process any document chunks due to API issues." }, { status: 500 })
    }
    ...
    `
        ...
        console.error("Failed querying vector database/embeddings:", e)
        return NextResponse.json({ error: "Failed retrieving relevant document context." }, { status: 500 })
    }

    // Step 5: Send relevant chunks to Groq
    try {
