...
export default function BoardPage() {
    ...
    useEffect(() => {
        if (chatOpen && !chatHistoryLoaded.current) {
        ...
        <div className="min-h-screen p-8">
            ...
            {!chatOpen && (
                <button
                    ...
                    className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-linear-to-br from-blue-500 to-purple-600 text-white text-2xl shadow-lg hover:scale-110 transition-all z-40"
                >
                    🤖
                </button>
            )}
            ...
            {/* Chat panel */}
            {chatOpen && (
                <div className="fixed right-0 top-0 h-full w-96 backdrop-blur-md bg-white/10 border-l border-white/20 flex flex-col z-30 shadow-2xl">

                    {/* Header with close button */}
                    <div className="p-4 border-b border-white/20 flex items-center justify-between">
                        <div>
                            <h2 className="text-white font-bold text-lg">🤖 Flowbit AI</h2>
                            <p className="text-white/50 text-xs">Ask anything about this project</p>
                        </div>
                        <button
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                        {chatHistory.length === 0 && (
                            <div className="space-y-2">
                                ...
                                {["What tasks are overdue?", "What should we prioritize?", "Summarize the sprint", "Who has the most tasks?"].map(q => (
                                    <button
                                        key={q}
                                        onClick={() => { setChatMessage(q); }}
                                        className="block w-full text-left text-xs text-white/60 bg-white/10 border border-white/20 rounded-lg px-3 py-2 hover:bg-white/20 transition-all"
                                    >
                                        {q}
                                    </button>
                                ))}
                        ...
                        {chatLoading && (
                            <div className="flex justify-start">
                                <div className="bg-white/10 rounded-2xl px-4 py-2 text-white/60 text-sm animate-pulse">
                                    Thinking...
                                </div>
                            </div>
                        )}
                    ...
                    <div className="p-4 border-t border-white/20 flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-white/10 border border-white/20 rounded-full px-4 py-2 text-white placeholder-white/40 focus:outline-none text-sm"
                            style={{ color: "white" }}
                        />
                        <button
                            onClick={handleChat}
                            disabled={chatLoading}
                            className="w-10 h-10 rounded-full bg-blue-500/30 border border-blue-500/40 text-white hover:bg-blue-500/50 transition-all disabled:opacity-50"
                        >
                            →
                        </button>
                    </div>
